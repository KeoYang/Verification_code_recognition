		...
		try {
			...
			for (File file : files) {
				...
				for(int j=0; j<list.size(); j++){
					...
					String filename = "4_drop/" + prex + "-" + j + ".jpg";
...
//		}
		...
		int num = (int)Math.round((double)width/meanD);//字符个数
		int lastP = 0; //上一次分割的位置
		int curSplit = 1;//分割的个数，小于等于num-1
...
//					System.out.println((x - left) + ", " + (start.y - top));
	...
	private int getPixelValue(int cx, int cy, int j){
		...
		//如果1<= j <= 3, 则判断下方的区域，只要有一个黑点，则当做黑点
		int start = cx - b + j - 2;
		int end = cx + b  + j - 2;
