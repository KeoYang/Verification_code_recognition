...
public class SegCfg {
	...
	private static void test(){
		try {
			...
			File file = new File("jiangsu_cfs/197-0.jpg");
	...
	public ArrayList<BufferedImage> cfs(BufferedImage sourceImage){
		...
		HashMap<String, Boolean> trackMap = new HashMap<String, Boolean>(); //以及访问过的点
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				int rgb = sourceImage.getRGB(x, y);
				String key = x + "-" + y;
				//如果不是黑色，或者以及被访问过，则跳过Cfg
				if (!isBlack(rgb) || trackMap.containsKey(key)) {
					continue;
				}
				
				//如果是黑色，且没被访问，则以此点开始进行连通域探索
				SubImage subImage = new SubImage();//保存当前字符块的坐标点
				LinkedList<Point> queue = new LinkedList<Point>();//保存当前字符块的访问队
				...
				if(subImage.width < minW || subImage.height < minH){
					continue;
				}
